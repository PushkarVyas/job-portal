...
export const registerCompany = async (req, res) => { 
    try{
        ...
        if(!companyName){
            return res.status(400).json({message: "comapany name is required", success: false});
        };

        let company = await Company.findOne({name: companyName})
        if(company){
            return res.status(400).json({message: "you cant register same company twice", success: false});
        };
